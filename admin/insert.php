...
// Get the form data
    ...
    $i_price = mysqli_real_escape_string($conn, $_POST['i_price']);
...
if (is_existing($conn, $item_name, 'item_name', 'item')) {
    ...
    if (isset($_FILES["item_file"])) {
        ...
        if ($uploadOk == 0) {
            ...
            if (!empty($item_name) && !empty($i_price)) {
                // Insert the new item's information into the item table
                $query = "INSERT INTO item (item_name, item_file, i_price) VALUES ('$item_name', '$target_file', '$i_price')";
                mysqli_query($conn, $query);
