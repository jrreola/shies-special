...
<body>
  ...
  <!-- PRODUCT DISPLAY -->
  <div class="container">
            <div class="col-md-12 bg-transparent;"> <!-- Displaying the list of products -->
              <?php
                if (isset($_GET['search'])) {
                  //sanitize and store the search query
                  $searchkey = htmlentities($_GET['search']);
                ...
                $stmt = $db->prepare(
                  "SELECT i.item_id, p.price_id, c.cat_id, s.size_id, i.item_name, c.category_name, c.category_description, s.size, p.item_price, c.category_name, c.cat_file, i.i_price
                  FROM price p
                  INNER JOIN category c ON p.cat_id = c.cat_id
                  INNER JOIN item i ON i.item_id = p.item_id
                  INNER JOIN sizes s ON s.size_id = p.size_id
                ...
                if(isset($_GET['search'])){
                  $searchkey=htmlentities($_GET['search']);
                  $sql_item = "SELECT item_id, item_name, item_file, date_added, i_price 
                              FROM item
                              WHERE item_stats = 'A' AND item_name LIKE '%{$searchkey}%'";
              }
              // query for the CATEGORY NAVIGATION bar
              else if (isset($_GET['cat_id'])){
                  $sql_item = "SELECT item_id, item_name, item_file, i_price
                              FROM item
                              WHERE item_stats = 'A' AND cat_id" ;
              } else {
              ...
              $item = $stmt_item->fetchAll(PDO::FETCH_ASSOC);
                    ...
                    if (count($item) > 0) {
                        echo '<div class="col-12" style="color: black;"><hr>';
                        echo '<h2 class="mt-5" style="background: white; text-align: center;">' . '</h2>';
                        echo '<div class="row">';
                    }

                    // Loop through the products of the current category
                    foreach ($item as $row) {
                      if ($row['item_id'] == 1 && $row['item_name'] == "Pancit Malabon") {
                        continue;
                      }                
                        echo '<div class="col-md-3 border border-white ms-5" style="background: white; margin: 10px;">';
                        echo '<div class=" mt-4">';
                        echo '<img style="border: solid black 2px; max-width: 100%; " src="img/' . $row['item_file'] . '" alt="' . $row['item_name'] . '">';
                        ...
                        if (isset($row['category_description'])) {
                          echo '<h3>' . $row['category_description'] . '</h3>';
                        }
                        if (isset($row['size'])) {
                          echo '<!-- <b><p>Size: ' . $row['size'] . '</p></b>  -->';
                        }
                        // if (isset($row['i_price'])) {
                        //   echo '<b><p>Price:' . $row['i_price'] . '</p></b>';
                        // }                        
                        ...
                        // }
                        echo '<span class="input-group-text product">Price</span>';
                        if (isset($row['i_price'])) {
                            echo '<input type="text" readonly="readonly" style="border-color: #333;" class="form-control" value="'. $row['i_price'] .'"/>';
                        }
                        echo '</div>';
                        echo '<div class="input-group mb-3">';
                        echo '<span class="input-group-text product">Quantity</span>';
                        echo '<input type="number" style="border-color: #333;" class="form-control" name="item_qty" value="1" min="1"/>';
                        echo '</div>';
                        echo '<button type="submit" class="btn btn-warning mt-2 mb-2">Add to cart</button>';
